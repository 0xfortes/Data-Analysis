...
def calculate_demographic_data(print_data=True):
  ...
  above = len(higher_education[higher_education.salary == '>50K'])

  lower_education = df[~df['education'].
                       isin(['Bachelors', 'Masters', 'Doctorate'])]
  below = len(lower_education[lower_education.salary == '>50K'])

  # percentage with salary >50K
  higher_education_rich = round(above / len(higher_education) * 100, 1)
  lower_education_rich = round(below / len(lower_education) * 100, 1)
  ...
  rich_percentage = rich_percentage = round(
  ...
  occupation = people_of_india['occupation'].value_counts()
